...
export function toggleForm(post: any = null) {
    ...
    if (form.style.display === 'block') {
      ...
      (document.getElementById('postForm') as HTMLFormElement).reset()
      ...
      if (post) {
        ...
        ;(document.getElementById('postContent') as HTMLTextAreaElement).value = post.content
        ;(document.getElementById('postCategory') as HTMLSelectElement).value = post.category || ''
  ...
  export async function connectPhantom() {
    ...
    if (window.solana?.isPhantom) {
      try {
        ...
        phantomBtn.innerHTML = `<i class="fas fa-wallet"></i> ${pk.slice(0,4)}...${pk.slice(-4)}`
  ...
  export async function participatePresale() {
    const amt = parseFloat((document.getElementById('presaleAmount') as HTMLInputElement).value)
  ...
  export function sortPosts(type: string, period: string | null = null) {
    document.querySelectorAll('.sort button:not(.stake-btn)').forEach(b => b.classList.remove('active'))
  ...
  export async function stakeTokens() {
    if (!window.solana?.publicKey) return showTooltip('Connect Phantom first')
    const amt = parseFloat((document.getElementById('stakeAmount') as HTMLInputElement).value)
